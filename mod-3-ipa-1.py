...
def scytale_cipher(message, shift):
    ...
    for element, character in enumerate(message):
        new_index = (element//shift) + (len(message) // shift) * (element%shift)
        character = message[new_index]
        scytale = scytale + character
...
def scytale_decipher(message, shift):
    ...
    while (len(decipher)<len(message)):
        decipher = decipher + '_'
        if len(decipher)==len(message):
            break
        
    for i, j in enumerate(message):
        new_index = (i//shift) + (len(message) // shift) * (i%shift)
        for k, l in enumerate(decipher):
            decipher = decipher[:new_index] + j + decipher[new_index + 1:]
